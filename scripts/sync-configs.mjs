...
const syncConfigs = () => {
    try {
        console.log(`Syncing configuration from Kong admin to ${OUTPUT_FILE}`);
        ...
        const stdout = execSync(`kubectl create configmap ${CONFIGMAP_NAME} --from-file=${OUTPUT_FILE} --dry-run=client -o yaml`);
